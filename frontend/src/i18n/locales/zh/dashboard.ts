export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      copy: '复制',
      copied: '已复制',
      setupModeRecommended: '推荐配置',
      setupModeAdvanced: '高级配置',
      quickSetup: {
        description: '选择客户端和模型后生成可直接使用的配置，系统会自动处理 /v1 等端点差异。',
        client: '客户端',
        model: '可用模型',
        modelPlaceholder: '输入或选择模型 ID',
        modelLoading: '正在读取当前密钥的可用模型...',
        modelLoaded: '已从当前密钥读取 {count} 个模型，也可以手动输入模型 ID。',
        modelLoadFailed: '未能读取模型列表，仍可手动输入模型 ID。',
        refreshModels: '重新读取模型',
        protocol: '接口协议',
        responses: 'OpenAI Responses API',
        chat: 'OpenAI Chat Completions',
        protocolFixed: '当前分组使用原生协议，配置已自动选择。',
        operatingSystem: '操作系统',
        unix: 'macOS / Linux',
        windows: 'Windows',
        claudeMode: 'Claude 模型映射',
        claudeSingle: '固定一个模型',
        claudeFamily: '分别映射 Opus / Sonnet / Haiku',
        claudeSingleHint: '三个 Claude 系列请求都会映射到当前选择的模型。',
        claudeFamilyHint: '留空的系列不会写入配置，由客户端模型名和服务端分组映射决定。',
        opus: 'Opus 映射模型',
        sonnet: 'Sonnet 映射模型',
        haiku: 'Haiku 映射模型',
        optionalModel: '可留空',
        mergeJson: '这是完整 JSON 示例；已有配置时请合并对应字段，不要覆盖无关设置。',
        mergeToml: '这是 TOML 配置区块；已有文件时请合并区块并避免重复名称。',
        codexEnv: '密钥通过环境变量提供，请在启动 Codex 前执行。',
